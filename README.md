...
> 이 문서는 마일스톤이 진행될 때마다 함께 갱신됩니다. 현재 상태: **M5(검수) 완료**
...
- **구글 로그인**: 매직링크만 우선 구현했다. Supabase Auth는 provider를 나중에 추가해도 기존 로그인 흐름에 영향이 없어서, 필요할 때 Google Cloud Console에서 OAuth 클라이언트를 발급받아 붙이면 된다.
